                    ...
                    <div class="row mb-4">
                        <div class="col-12">
                            <h6 class="text-primary mb-3">
                                <i class="fas fa-home"></i> Select Household
                            </h6>
                        </div>
                            ...
                            <select class="form-select" id="household_id" name="household_id" required>
                                ...
                                <?php foreach ($households as $household): ?>
                                    <option value="<?= $household['id'] ?>" 
                                            <?= ($_POST['household_id'] ?? '') == $household['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($household['household_head']) ?> - 
                                        <?= htmlspecialchars($household['control_number']) ?>
                                        (<?= $household['member_count'] ?> members)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <small class="text-muted">Select the household to add this family member to</small>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const specialNeedsGroup = document.getElementById('special_needs_description_group');
    
    hasSpecialNeeds.addEventListener('change', function() {
        specialNeedsGroup.style.display = this.checked ? 'block' : 'none';
    });
    
    // Show on page load if already checked
    if (hasSpecialNeeds.checked) {
        specialNeedsGroup.style.display = 'block';
    }
});
</script>
